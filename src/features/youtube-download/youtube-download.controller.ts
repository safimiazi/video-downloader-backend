...
export class YoutubeDownloadController {
  ...
  constructor(private readonly youtubeDownloadService: YoutubeDownloadService) {}
   ...
   * @private
   */
  private async handleDirectDownload(query: DownloadRequestDto, res: Response): Promise<void> {
    ...
    if (result.buffer && result.metadata) {
      ...
      this.logger.log(`Streaming file: ${result.metadata.filename} (${result.metadata.fileSize} bytes)`);
   ...
   * @private
   */
  private async handleProgressiveDownload(query: DownloadRequestDto, res: Response): Promise<void> {
    ...
    try {
      ...
      const progressStream = this.youtubeDownloadService.createProgressiveDownloadStream(query);
